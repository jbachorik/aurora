...
public final class SeriesFolders {
     ...
     *
     * <p>Season and episode may be parted by one space, dot, underscore or
     * dash. {@code S01.E01} is what a good many rippers write, and reading that
     * as two unrelated numbers left the folder queueing nothing at all: the run
     * stopped dead after its first episode, with nothing on screen to say why.
     * One separator character only, and an episode number has to follow it, or
     * "Series 1 - Episode 1" would start counting as a tag.
     */
    private static final Pattern EPISODE_TAG = Pattern.compile(
            "(?i)(?<![\\p{L}\\p{N}])(?:s\\d{1,2}[\\s._-]?e\\d{1,3}|\\d{1,2}x\\d{2,3})(?![\\p{L}\\p{N}])");
